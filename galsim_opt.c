...
void add_to_quadtree(galquad_t** __restrict tree, particle_t* __restrict particle, double x, double y, double size)
{
  double half = size/2;
...
void initialize_graphics(char* __restrict prog, particle_t* __restrict particles, double* __restrict br, int N)
{
  int i;
...
void print_particles(particle_t* __restrict particles, double* __restrict br, int N)
{
  int i;
  for(i=0; i<N; ++i)
  {
...
double find_theta(particle_t *p, galquad_t* t)
{
  ...
  double dx, dy;
  dx = t->xstart + half - p->x; //x distance between centre of quad and particle
  dy = t->ystart + half - p->y; //y distance between centre of quad and particle
...
void force(particle_t* p, galquad_t* t, double* fx, double* fy, double theta_max)
{
  ...
  if(t->topleft != NULL)
  {
    ...
    {
      force(p, t->topleft, fx, fy, theta_max);
    }
  ...
  if(t->topright != NULL)
  {
    ...
    {
      force(p, t->topright, fx, fy, theta_max);
    }
    else //treat tree as a point mass
    {
      dx = (p->x - t->topright->x);
  ...
  if(t->bottomleft != NULL)
  {
    ...
    {
      force(p, t->bottomleft, fx, fy, theta_max);
    }
    else //treat tree as a point mass
    {
  ...
  if(t->bottomright != NULL)
  {
    ...
    {
      force(p, t->bottomright, fx, fy, theta_max);
    }
    else //treat tree as a point mass
    {
...
void sim_step(particle_t* __restrict particles, galquad_t* __restrict tree, int N, const double dt, const double G, const double theta_max)
{
  int i;
  ...
  {
    force(&particles[i], tree, &fx[i], &fy[i], theta_max);
    //printf("applying force %f, %f to particle %d\n", fx[i], fy[i], i);
    ax[i] = fx[i]*G*dt;
    ay[i] = fy[i]*G*dt;
    particles[i].vx = particles[i].vx + ax[i];
    particles[i].vy = particles[i].vy + ay[i];
