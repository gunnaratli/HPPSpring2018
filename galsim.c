...
int main(int argc, char** argv)
{
  ...
  for(i=0; i<N; ++i)
  {
    fread(&(particles[i].x), sizeof(double), 1, infile);
    fread(&(particles[i].y), sizeof(double), 1, infile);
    fread(&(particles[i].mass), sizeof(double), 1, infile);
    fread(&(particles[i].vx), sizeof(double), 1, infile);
    fread(&(particles[i].vy), sizeof(double), 1, infile);
    //read brightness into a separate array to keep particle memory usage smaller
    fread(&(br[i]), sizeof(double), 1, infile);
  }
  fclose(infile);
  //print_particles(particles, br, N);
  int graphics;
